...
public class ImageProcessingService
{
    ...
    public async Task StartProcessing(string targetDirectory, ProcessingModeEnum processingMode, bool markClassificationErrors, CancellationTokenSource cancellationTokenSource, LanguageEnum language = LanguageEnum.German)
    {
        ...
        //check connection to ollama service first
        try
        {
            var response = await client.GetAsync("http://localhost:11434", cancellationTokenSource.Token);
            response.EnsureSuccessStatusCode();
        }
        catch (Exception ex)
        {
            throw new Exception("Could not connect to Ollama service. Please ensure it is running.", ex);
        }
